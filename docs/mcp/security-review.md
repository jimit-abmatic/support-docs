...
| Question | Answer |
|----------|--------|
| What is logged? | A per-request audit line for every authenticated API request (account id, key id, endpoint, request id, response status, UTC timestamp), plus application logs to Amazon CloudWatch, load balancer access logs to Amazon S3, and platform API logs. Every API response also carries the same `request_id` that appears in its audit line, so a call can be traced end to end. |
| Is key access auditable? | Yes. Every authenticated request is recorded as described above. No part of the key is logged, not even its display prefix (the audit line carries a one way key id instead), and the endpoint is recorded as the route rather than the resolved path or query string, so the audit trail holds no customer data of its own. The tradeoff: the line identifies which endpoint a key called, not which individual record it returned. The resolved path is in the load balancer access logs, correlated by the same request id. |
| Is a customer facing audit trail available? | The audit log is server side, not self serve. Contact [support@abmatic.ai](mailto:support@abmatic.ai) for the access history of a specific window or key. Surfacing it in the dashboard is roadmap work. |
